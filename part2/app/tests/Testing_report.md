...
| Filter by email | `?email=jane.filter@example.com` | 200 + matching user | PASS |

Amenity Model
| Test | Description | Result |
|------|-----------------|--------|
| Filter by non-existent email | `?email=nobody@nowhere.com` | 404 | PASS |
| Get user by valid ID | Valid UUID | 200 + user | PASS |
| Get user by invalid ID | `nonexistent-id` | 404 | PASS |
